...
- New user-facing features ship behind a PostHog feature flag. (1) Register the flag key in `FEATURE_FLAGS` in `src/composables/useFeatureFlags.ts` (kebab-case, area prefix such as `beta-`/`editor-`/`pro-`). (2) Gate the entry point with `useFeatureFlags().isEnabled(FEATURE_FLAGS.YOUR_FLAG)` — new tile types follow the `document`/`roadmap_feed` pattern (a `featureFlag` on the `TileDefinition` plus a flag-gated toolbar button). (3) Create the matching flag in the PostHog dashboard before rollout — the registry entry alone does not create it; locally, flags default to enabled in dev when no `VITE_POSTHOG_KEY` is set.
- New flags are created **active/enabled but gated to the team during beta**: a single release condition targeting person property `email` `icontains` `@trustybits` at 100% rollout. This lets the team dogfood in production immediately while the public stays gated; widen the targeting (cohort, or 100% to everyone) only when the feature is ready for general rollout. Tag beta flags with `beta`.
