...
const { mockGridStore, mockUseGridController } = vi.hoisted(() => {
  const mockGridStore = {
    addTile: vi.fn(),
  ...
  return {
    mockGridStore,
    mockUseGridController: vi.fn(() => mockGridStore),
  };
});
...
describe("setup", () => {
  it("does not resolve the live grid controller until content is applied", async () => {
