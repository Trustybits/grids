...
Production-oriented Firebase runtime code for `grids.so` — the backend implementations behind the `@grids/contracts` interfaces. This package lives in the public repository. It is designed primarily for production Firebase config, but contributors can use it with local Firebase emulators. When Firebase config is absent or invalid, `apps/web` falls back to its local stubs.
...
- **`src/runtime/`** — the composition root.
  ...
  - `firebaseConfigs.ts` — per-environment Firebase project config, loaded from a **gitignored `firebaseConfigs.json`** sitting next to it via `import.meta.glob` (bundle-time optional import). The JSON is present in production/deploy environments and absent from ordinary local checkouts; when absent the glob yields nothing, `hasFirebaseConfig` is `false`, and the app falls back to the stubbed backend instead of crashing. Contributors can generate demo local config with `npm run emulators:setup`. The build copies the JSON into `dist/runtime/` so the glob resolves there (`apps/web` sets `optimizeDeps.exclude: ['@grids/pro']` so Vite — not esbuild — transforms the glob in dev). **Note:** the `stage` config is still `REPLACE_ME` placeholders; selecting `stage` won't connect until those are filled in from the Firebase console.
