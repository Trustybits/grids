...
export const startMatricksBackground = (
  ...
  const pickTileUnitSize = () => {
    if (gridsTileSizes.length > 0) {
      return clampTileUnitSize(
        gridsTileSizes[Math.floor(Math.random() * gridsTileSizes.length)],
      );
    }

    const w =
      gridsUnitChoices[Math.floor(Math.random() * gridsUnitChoices.length)];
    const h =
      gridsUnitChoices[Math.floor(Math.random() * gridsUnitChoices.length)];
  ...
  const gridsTileGapChars = Math.max(
    0,
    Math.round(gridsTileGapUnits * gridsUnitSizeChars),
  );

  const snapToGrid = (pos: number) => {
    return Math.max(
      0,
      Math.round(pos / gridsPlacementStepChars) * gridsPlacementStepChars,
    );
  };

  const clampSnappedPos = (pos: number, maxPos: number) => {
    const maxSnapped = Math.max(
      0,
      Math.floor(maxPos / gridsPlacementStepChars) * gridsPlacementStepChars,
    );
  ...
  const pickSnappedPos = (maxPos: number) => {
    const maxSnapped = Math.max(
      0,
      Math.floor(maxPos / gridsPlacementStepChars),
    );
    return (
      Math.floor(Math.random() * (maxSnapped + 1)) * gridsPlacementStepChars
    );
  };
  ...
  const getTileSizeAtFrame = (tile: GridsTile, frame: number) => {
    ...
    const t = Math.min(
      1,
      Math.max(0, (frame - tile.transitionStartFrame) / denom),
    );
    const eased = t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;
    const w = Math.max(
      1,
      Math.round(tile.startW + (tile.targetW - tile.startW) * eased),
    );
    const h = Math.max(
      1,
      Math.round(tile.startH + (tile.targetH - tile.startH) * eased),
    );
    return { w, h };
  };
  ...
  if (animationPattern === "grids") {
    ...
    while (
      gridTiles.length < gridsTileCount &&
      globalAttempts < gridsTileCount * perTileAttempts
    ) {
      globalAttempts++;
      ...
      gridTiles.push({
        ...
        nextChangeFrame:
          Math.floor(Math.random() * gridsTileChangeEveryFrames) +
          gridsTileChangeEveryFrames,
  ...
  const maxMessageLength =
    emphasizedMessages.length > 0
      ? Math.max(
          ...emphasizedMessages.map(getMaxLineLength),
    ...
    for (let row = 0; row < rows; row++) {
      const animationTypes: GridCell["animationType"][] =
        animationPattern === "grids"
          ? ["pulse"]
          : ["pulse", "flicker", "wave", "strobe", "fade"];

      const isEmphasizedCell =
        !!emphasizedText &&
        row === textStartY &&
        col >= textStartX &&
        col < textStartX + (emphasizedText?.length || 0);

      const emphasizedChar = isEmphasizedCell
        ? emphasizedText[col - textStartX]
        : null;
  ...
  const draw = () => {
    ...
    if (animationPattern === "rain") {
      for (let col = 0; col < columns; col++) {
        ...
        if (rainColumn.active) {
          ...
          if (Math.random() < 0.02 * patternSpeed) {
            // Activation chance affected by patternSpeed
      ...
      for (const tile of gridTiles) {
        if (frameCount >= tile.nextChangeFrame) {
          ...
          for (let attempt = 0; attempt < attemptCount; attempt++) {
            ...
            const nextX = wantsMove
              ? pickSnappedPos(maxX)
              : clampSnappedPos(tile.x, maxX);
            const nextY = wantsMove
              ? pickSnappedPos(maxY)
              : clampSnappedPos(tile.y, maxY);
          ...
          if (!didUpdate) {
            ...
            tile.nextChangeFrame =
              frameCount + Math.floor(gridsTileChangeEveryFrames / 2);
          }
        }
      }
    ...
    for (let col = 0; col < columns; col++) {
      for (let row = 0; row < rows; row++) {
        ...
        const tileId =
          animationPattern === "grids" && gridsTileLookup
            ? gridsTileLookup[col][row]
            : -1;
        ...
        if (!cell.isEmphasized) {
          ...
          switch (animationPattern) {
            case "rain":
              ...
              if (rainColumn.active) {
                ...
                const tailRow = Math.floor(
                  rainColumn.headPosition - rainColumn.length,
                );
                ...
                // Characters in the trail change less frequently
                  ...
                  changeChance =
                    0.3 *
                    Math.max(0.2, 1 - distanceFromHead / rainColumn.length);
                }
            ...
            case "wave":
              ...
              changeChance =
                (Math.sin(frameCount * 0.05 + row * 0.2) + 1) * 0.2;
              break;
            ...
            case "ripple":
              ...
              for (const ripple of rippleCenters) {
                const distance = Math.sqrt(
                  Math.pow(col - ripple.x, 2) + Math.pow(row - ripple.y, 2),
                );
          ...
          if (
            frameCount - cell.lastChange >
            (60 + Math.random() * 120) / animationSpeed
          ) {
        ...
        switch (animationPattern) {
          case "rain":
            ...
            if (rainColumn.active) {
              ...
              // Characters in the trail have decreasing intensity
                ...
                cell.intensity = Math.max(
                  0.2,
                  1 - (distanceFromHead / rainColumn.length) * 0.8,
                );
              }
            ...
            cell.phase +=
              cell.speed *
              (rainColumn.active && row <= Math.floor(rainColumn.headPosition)
                ? 1.5
                : 0.3);
            break;
          ...
          case "ripple":
            ...
            for (const ripple of rippleCenters) {
              ...
              const fadeFactor =
                ripple.phase < 8 ? 1 : Math.max(0, 1 - (ripple.phase - 8) / 7);
        ...
        switch (cell.animationType) {
          ...
          case "wave":
            ...
            opacity =
              (Math.sin(cell.phase + waveOffset) * 0.5 + 0.5) * cell.intensity;
            // green = Math.floor(255 * opacity);
            break;

          case "strobe":
            opacity =
              Math.sin(cell.phase * 8) > 0.7
                ? 0.9 * cell.intensity
                : 0.1 * cell.intensity;
            // green = Math.floor(255 * opacity);
            break;
          ...
          case "fade":
            ...
            opacity =
              (Math.sin(fadePhase * Math.PI) * 0.8 + 0.2) * cell.intensity;
            // green = Math.floor(255 * opacity);
            break;
        }
        ...
        if (!cell.isEmphasized) {
          switch (animationPattern) {
            ...
            case "wave":
              ...
              opacity *=
                0.5 + Math.abs(Math.sin(frameCount * 0.02 + row * 0.2)) * 0.5;
              break;
            ...
            case "ripple":
              ...
              for (const ripple of rippleCenters) {
                ...
                const fadeFactor =
                  ripple.phase < 8
                    ? 1
                    : Math.max(0, 1 - (ripple.phase - 8) / 7);

                if (proximity < 2) {
                  const effectStrength = ((2 - proximity) / 2) * fadeFactor;
                  maxRippleIntensity = Math.max(
                    maxRippleIntensity,
                    effectStrength,
                  );
                }
        ...
        if (cell.isEmphasized) {
          ...
          const pulse = emphasizedPulse
            ? Math.sin(frameCount * 0.05 * animationSpeed) * 0.3 + 0.7
            : 1;
          const glow = emphasizedGlow
            ? Math.sin(frameCount * 0.03 * animationSpeed) * 0.3 + 0.7
            : 0.5;
          ...
          if (rippleEffect > 0.3) {
            ...
            const lowRGB = rippleLowColor
              .split(",")
              .map((c) => parseInt(c.trim()));
            const highRGB = rippleHighColor
              .split(",")
              .map((c) => parseInt(c.trim()));
            const interpolatedR = Math.floor(
              lowRGB[0] + (highRGB[0] - lowRGB[0]) * rippleEffect,
            );
            const interpolatedG = Math.floor(
              lowRGB[1] + (highRGB[1] - lowRGB[1]) * rippleEffect,
            );
            const interpolatedB = Math.floor(
              lowRGB[2] + (highRGB[2] - lowRGB[2]) * rippleEffect,
            );

            ctx.fillStyle = `rgba(${interpolatedR}, ${interpolatedG}, ${interpolatedB}, ${opacity})`;
          } else if (
            animationPattern !== "grids" &&
            Math.random() < flashFrequency
          ) {
        ...
        if (
          ...
          const n =
            ...
            (tick * 1442695041);
          const tileChar =
            charArray[hashU32(n) % Math.max(1, charArray.length)];
          cell.char = tileChar;

          const tilePulse =
            Math.sin((frameCount + tile.seed) * 0.012) * gridsTileOpacityPulse;
          const tileOpacity = Math.min(
            0.85,
            Math.max(0.05, gridsTileOpacityBase + tilePulse),
          );
    ...
    if (emphasizedMessages.length > 0) {
      ...
      switch (animationState) {
        case "typing":
          ...
          if (typingProgress < message.length) {
            typingProgress = Math.min(
              typingProgress +
                (currentTime - lastMessageTime) / (1000 / typingSpeed),
              message.length,
            );
        ...
        case "fadeOut":
          ...
          fadeOutProgress = Math.min(
            (currentTime - lastMessageTime) / fadeOutDuration,
            1,
          );
      ...
      for (let col = 0; col < columns; col++) {
        for (let row = 0; row < rows; row++) {
          ...
          for (
            let lineIndex = 0;
            lineIndex < Math.max(messageLines.length, 5);
            lineIndex++
          ) {
            const lineY =
              textStartY +
              Math.floor(
                (lineIndex * lineHeight * fontSize) / effectiveFontSize,
              );
            if (row === lineY) {
              // For center alignment, check within the centered area
              if (textAlignment === "center") {
                const potentialMaxWidth = Math.max(
                  ...emphasizedMessages.map(getMaxLineLength),
                );
                const clearStartX = Math.round(centerX - potentialMaxWidth / 2);
                if (
                  col >= clearStartX &&
                  col < clearStartX + potentialMaxWidth
                ) {
                  isMessageCell = true;
                  break;
                }
              ...
              // For left alignment, check from the start position
                ...
                col >= textStartX &&
                col < textStartX + maxMessageLength
              ) {
                isMessageCell = true;
                break;
              }
      ...
      for (let lineIndex = 0; lineIndex < messageLines.length; lineIndex++) {
        ...
        const lineStartY =
          textStartY +
          Math.floor((lineIndex * lineHeight * fontSize) / effectiveFontSize);
