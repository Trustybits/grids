...
describe("getUserProfile", () => {
  it("returns the user profile when the document exists", async () => {
    ...
    vi.mocked(firestore.getDoc).mockResolvedValueOnce(
      mockDocSnap(true, profile) as any,
    );
  ...
  it("throws when Firestore throws an error", async () => {
    ...
    await expect(getUserProfile("uid-abc")).rejects.toThrow(
...
describe("checkSlugAvailability", () => {
  it("returns available response from Cloud Function", async () => {
    ...
    vi.mocked(functions.httpsCallable).mockReturnValueOnce(mockCallable as any);
  ...
  it("throws with a user-friendly message when Cloud Function fails", async () => {
    const mockCallable = vi
      .fn()
      .mockRejectedValueOnce({ message: "Functions error" });
    vi.mocked(functions.httpsCallable).mockReturnValueOnce(mockCallable as any);

    await expect(checkSlugAvailability("slug")).rejects.toThrow(
      "Functions error",
    );
  });
});
...
describe("claimSlug", () => {
  ...
  it("throws with error message when claim fails", async () => {
    const mockCallable = vi
      .fn()
      .mockRejectedValueOnce({ message: "Slug already taken" });
    vi.mocked(functions.httpsCallable).mockReturnValueOnce(mockCallable as any);
...
describe("updateUserProfile", () => {
  it("calls setDoc with merge: true for partial updates", async () => {
    vi.mocked(firestore.doc).mockReturnValueOnce("dummy-ref" as any);
